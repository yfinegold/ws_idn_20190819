...
try:
    attr
    rasterize = "gdal_rasterize -a %s -l %s -ot UInt16 -te %r %r %r %r -tr %r %r -co \"COMPRESS=LZW\" %s %s/tmp_mask.tif\n" % (attr,layer,im_xmin,im_ymin,im_xmax,im_ymax,pxsz,pxsz,vector,tmpdir)
except NameError:
    print "-"*40 + "\nNo attribute specified, will clump all features of %s" % vector
    rasterize = "gdal_rasterize -clump -l %s -ot UInt16 -te %r %r %r %r -tr %r %r -co \"COMPRESS=LZW\" %s %s/tmp_mask.tif\n" %   (layer,im_xmin,im_ymin,im_xmax,im_ymax,pxsz,pxsz,vector,tmpdir)
...
################# Compress and output
compress = "gdal_translate -ot UInt16 -co \"COMPRESS=LZW\" %s/tmp_mask.tif %s" % (tmpdir,outfile) 
print "-"*40 + "\n" + compress
os.system(compress)
